...
if(trim($name_contact) == '') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> You must enter your Name.</div>';
	exit();
} else if(trim($lastname_contact ) == '') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> You must enter your Last name.</div>';
	exit();
} else if(trim($email_contact) == '') {
	echo '<div class="error_message"><i class=" icon-attention-2"></i> Please enter a valid email address.</div>';
	exit();
} else if(!isEmail($email_contact)) {
	echo '<div class="error_message"><i class="icon-attention-2"></i> You have enter an invalid e-mail address, try again.</div>';
	exit();
	} else if(trim($phone_contact) == '') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> Please enter a valid phone number.</div>';
	exit();
} else if(!is_numeric($phone_contact)) {
	echo '<div class="error_message"><i class=" icon-alert"></i> Phone number can only contain numbers.</div>';
	exit();
} else if(trim($message_contact) == '') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> Please enter your message.</div>';
	exit();
} else if(!isset($verify_contact) || trim($verify_contact) == '') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> Please enter the verification number.</div>';
	exit();
} else if(trim($verify_contact) != '4') {
	echo '<div class="error_message"><i class="icon-attention-2"></i> The verification number you entered is incorrect.</div>';
	exit();
}
...
$user = "$email_contact";
$usersubject = "Thank You for contact ACTIVITY";
$userheaders = "From: info@activity.com\n";
$usermessage = "Thank you for contact ACTIVITY. We will reply shortly!";
mail($user,$usersubject,$usermessage,$userheaders);

if(mail($address, $e_subject, $msg, $headers)) {

	// Success message
	echo "<div id='success_page' style='padding:20px 0 20px 0'>";
